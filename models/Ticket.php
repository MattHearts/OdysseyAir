...
class Ticket
{
    ...
    // Registers passenger's info for go flight into the database
    function register_passengers($x)
    {
        ...
        $flightType = "go";
        $checkedIn = "false";
        $stmt->bind_param("iissssssssi", $this->flightID, $this->bookingID, $this->passengerName, $this->passengerSurname, $this->passengerTitle, $this->passengerSeat, $this->insurance, $flightType, $checkedIn, $this->checkinType, $this->baggageNum);
        $result = $stmt->execute();
        if ($result) {
    ...
    // Registers passenger's info for return flight  into the database
    ...
    // Registers the booking into the database
    function register_booking()
    {
        ...
        $registerquery = "INSERT INTO bookings (username, date, time,price) VALUES (?,?,?,?)";
        $stmt = $conn->prepare($registerquery);

        $stmt->bind_param("sssi", $this->username, $date, $time, $price);
        $result = $stmt->execute();
    ...
    function show_booking()
    {
        ...
        $bookingquery = "SELECT * FROM flights WHERE flight_id=?";
        $stmt = $conn->prepare($bookingquery);
        ...
        $bookingquery = "SELECT * FROM passengers WHERE booking_id=? AND flight_id=?";
        $stmt = $conn->prepare($bookingquery);
        $stmt->bind_param("ii", $this->bookingID, $this->flightID);
        $stmt->execute();
        $result = $stmt->get_result();
        if ($result->num_rows > 0) {
            $x = 0;
        ...
        if ($_SESSION['flightType'] == "return") {
            $bookingquery = "SELECT * FROM passengers WHERE booking_id=? AND flight_id=?";
            $stmt = $conn->prepare($bookingquery);
            $stmt->bind_param("ii", $this->bookingID, $this->flightIDR);
            $stmt->execute();
            $result = $stmt->get_result();
            if ($result->num_rows > 0) {
                $x = 0;
